...
public class CollectorsAction {
    ...
    private static void testCollectingAndThen() {
        ...
        Optional.ofNullable(menu.stream().collect(Collectors.collectingAndThen(averagingInt(Dish::getCalories), a -> "The Average Calories is->" + a)))
                .ifPresent(System.out::println);
/*
    ...
    private static void testGroupingByFunction() {
        ...
        Optional.of(menu.stream().collect(groupingBy(Dish::getType)))
                .ifPresent(System.out::println);
    }

    private static void testGroupingByFunctionAndCollector() {
        System.out.println("testGroupingByFunctionAndCollector");
        Optional.of(menu.stream().collect(groupingBy(Dish::getType, averagingInt(Dish::getCalories))))
                .ifPresent(System.out::println);
    }
    ...
    private static void testGroupingByFunctionAndSupplierAndCollector() {
        ...
        Map<Dish.Type, Double> map = menu.stream().collect(groupingBy(Dish::getType, TreeMap::new, averagingInt(Dish::getCalories)));
